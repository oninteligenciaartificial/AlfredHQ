...
async function resolveWorkspace(sb: any, id: string): Promise<string> {
  const { data, error } = await sb.from('notes').select('workspace_id').eq('id', id).single()
  if (error || !data) throw new AppError('Note not found', 404, 'NOT_FOUND')
...
export async function PATCH(
  ...
  try {
    ...
    const allowed = ['title', 'body', 'pinned', 'business_id']
